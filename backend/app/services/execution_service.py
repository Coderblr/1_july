...
def execute_execution_run(
    ...
    def on_step_complete(result: dict) -> None:
        ...
        if result["status"] == "failed" and result.get("error") is not None:
        ...
        # "skipped" steps (assertion/unrecognized) are counted in total but
        # not as passed or failed — they represent steps the platform
        # encountered but deliberately did not execute.
        execution.total_steps += 1
        if result["status"] == "passed":
            execution.passed_steps += 1
        elif result["status"] == "failed":
            execution.failed_steps += 1
        # "skipped" increments total only; passed + failed + skipped = total
        if result["healed"]:
